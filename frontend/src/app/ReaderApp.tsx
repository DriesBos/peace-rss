...
export function ReaderApp({
  ...
  useEffect(() => {
    ...
    void bootstrap();
    // Bootstrap should only run until the server/client provisioned flag is true.
  }, [isProvisioned]);
  ...
  async function addFeed(payload: AddFeedPayload) {
    ...
    if (!trimmedUrl && !trimmedSelectedFeedUrl) {
      return { ok: false as const, error: 'Enter a feed URL.' };
    }
    if (payload.categoryId <= 0) {
      return { ok: false as const, error: 'Choose a category.' };
    }
  ...
  async function updateCategory(
    ...
    const category = categories.find((cat) => cat.id === payload.id);
    if (category && isProtectedCategoryTitle(category.title)) {
      toast.error('This category is managed automatically.');
      return false;
    }
    ...
    try {
      ...
      throw e instanceof Error ? e : new Error('Failed to update category');
    }
  }
  ...
  async function updateFeed(payload: UpdateFeedPayload): Promise<boolean> {
    ...
    try {
      ...
      throw e instanceof Error ? e : new Error('Failed to update feed');
    }
  }
